...
import { ADDNS_STEPS, ADDNS_MANIFEST_ID } from "@/lib/stepsAddns";
...
type StepId = "a1" | "a2" | "a3" | "a4" | "a5" | "a6" | "a7";
...
export default function Page() {
  ...
  const stepId = useMemo<StepId | null>(() => {
    ...
    const allowed = new Set<StepId>(["a1","a2","a3","a4","a5","a6","a7"]);
    return allowed.has(cleaned) ? cleaned : null;
  }, [params?.id]);

  const idx = useMemo(() => (stepId ? ADDNS_STEPS.findIndex(s => s.id === stepId) : -1), [stepId]);
  const step = idx >= 0 ? ADDNS_STEPS[idx] : undefined;

  const [map, setMap] = useState<Record<string, boolean>>({});
  useEffect(() => setMap(readMap(ADDNS_MANIFEST_ID)), []);
  useEffect(() => { if (!step) router.replace("/lab-addns"); }, [step, router]);

  const total = ADDNS_STEPS.length;
  const pct = useMemo(() => Math.round((ADDNS_STEPS.filter(s => map[s.id]).length / total) * 100), [map, total]);

  if (!step) return <main className="p-6">Lade…</main>;

  const isDone = !!map[step.id];
  const markDone = () => { const next = { ...map, [step.id]: true }; setMap(next); saveMap(ADDNS_MANIFEST_ID, next); };
    ...
    <main className="space-y-4">
      <div className="flex items-center justify-between">
        <div className="text-sm text-zinc-600">
          ...
          <Link href="/lab-addns" className="text-blue-600 hover:underline">AD / DNS / DHCP</Link>{" · "}
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="flex flex-wrap gap-2 pt-2">
            ...
            <Button className="border" onClick={() => router.push("/lab-addns")}>Zur Karte</Button>
            {prev && <Button className="border" onClick={() => router.push(`/lab-addns/step/${prev}`)}>◀ Zurück</Button>}
            {next && <Button className="border" onClick={() => router.push(`/lab-addns/step/${next}`)}>Weiter ▶</Button>}
          </div>
        </CardContent>
      </Card>
    </main>
