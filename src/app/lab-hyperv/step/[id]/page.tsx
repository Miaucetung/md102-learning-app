...
import { HYPERV_STEPS, HYPERV_MANIFEST_ID } from "@/lib/stepsHyperV";
...
export default function Page() {
  ...
  const stepId = useMemo<StepId | null>(() => {
    ...
    const allowed = new Set<StepId>(["t1","t2","t3","t4","t5"]);
    return allowed.has(cleaned) ? cleaned : null;
  }, [params?.id]);

  const idx = useMemo(() => (stepId ? HYPERV_STEPS.findIndex(s => s.id === stepId) : -1), [stepId]);
  const step = idx >= 0 ? HYPERV_STEPS[idx] : undefined;

  const [map, setMap] = useState<Record<string, boolean>>({});
  useEffect(() => setMap(readMap(HYPERV_MANIFEST_ID)), []);
  useEffect(() => { if (!step) router.replace("/lab-hyperv"); }, [step, router]);

  const total = HYPERV_STEPS.length;
  const pct = useMemo(() => Math.round((HYPERV_STEPS.filter(s => map[s.id]).length / total) * 100), [map, total]);

  if (!step) return <main className="p-6">Lade…</main>;

  const isDone = !!map[step.id];
  const markDone = () => { const next = { ...map, [step.id]: true }; setMap(next); saveMap(HYPERV_MANIFEST_ID, next); };
    ...
    <main className="space-y-4">
      <div className="flex items-center justify-between">
        <div className="text-sm text-zinc-600">
          ...
          <Link href="/lab-hyperv" className="text-blue-600 hover:underline">Hyper-V</Link>{" · "}
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="flex flex-wrap gap-2 pt-2">
            ...
            <Button className="border" onClick={() => router.push("/lab-hyperv")}>Zur Karte</Button>
            {prev && <Button className="border" onClick={() => router.push(`/lab-hyperv/step/${prev}`)}>◀ Zurück</Button>}
            {next && <Button className="border" onClick={() => router.push(`/lab-hyperv/step/${next}`)}>Weiter ▶</Button>}
          </div>
        </CardContent>
      </Card>
    </main>
